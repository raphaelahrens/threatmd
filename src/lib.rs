...
impl <'input> MarkdownParser<'input> {
    ...
    pub fn to_string(&self, range: Option<RangeInclusive<usize>>) -> String{
        range.map_or_else(|| String::new(), |r|{let mut s = String::new(); cmark((self.events[r]).iter(), &mut s).unwrap(); s},)
    }
...
impl <'inner> MarkdownIter<'inner> {
    ...
    pub fn heading(&mut self, level: HeadingLevel) -> Result<String, Error> {
        self.take(|e| {
            let &Event::Start(Tag::Heading{level:l,..}) = e else {return false;};
            l == level
        })?;
        let heading = self.take_text()?;
        self.take(|e| {
            let &Event::End(TagEnd::Heading(l)) = e else {return false;};
            l == level
        })?;
        Ok(heading)
    }

    pub fn named_heading(&mut self, level: HeadingLevel, content: &'static str) -> Result<(), Error> {
        if self.heading(level)? != content {
            return Err(Error::Expected(format!("A heading with the content: {}", content)));
        }
        Ok(())
    }
    ...
    pub fn paragraph(&mut self) -> Result<RangeInclusive<usize>, Error> {
        ...
        let Some((end, _)) = self.take_while(|(_i, x)| **x != Event::End(TagEnd::Paragraph)).last() else {
        ...
        Fun: Fn(&mut Self) -> Result<RangeInclusive<usize>, Error>
    {
        ...
        Err(Error::Expected("Expected either one of the functions to return".to_string()))
    }
        ...
        F: Fn(&Event) -> bool
    {
        ...
        let Some((end, _)) = self.take_while(|(_i, x)| **x != Event::End(TagEnd::CodeBlock)).last() else {
